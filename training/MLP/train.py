...
# Mathematically equivalent to a per-pixel MLP but natively accepts [1, C, H, W],
# which is the format the engine provides. Exports only Conv/ReLU/Sigmoid ONNX nodes.
...
def make_dataset(transform: Callable, num_samples: int) -> tuple[torch.Tensor, torch.Tensor]:
...
def save_comparison(model: ColorMLP, transform: Callable, image_path: str, out_path: str) -> None:
    ...
    with torch.no_grad():
        model_out = (model(x).squeeze(0).permute(1, 2, 0).numpy() * 255).astype(np.uint8)

    pixels_flat = torch.from_numpy(pixels_np).reshape(-1, 3)
    analytical = (transform(pixels_flat).numpy() * 255).astype(np.uint8).reshape(h, w, 3)

    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(18, 5))
    ax1.imshow(np.array(img));  ax1.set_title("Original");   ax1.axis("off")
    ax2.imshow(analytical);     ax2.set_title("Analytical");  ax2.axis("off")
    ax3.imshow(model_out);      ax3.set_title("Model");       ax3.axis("off")
    plt.tight_layout()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train a color-transform MLP and export to ONNX")
    p.add_argument(
        "--transform",
    ...
    p.add_argument("--epochs",      type=int,   default=100,    help="Training epochs (default: 400)")
    p.add_argument("--num-samples", type=int,   default=100_000, help="Random training pixels (default: 100 000)")
    p.add_argument("--batch-size",  type=int,   default=1024,    help="Batch size (default: 1024)")
    p.add_argument("--lr",          type=float, default=1e-3,    help="Learning rate (default: 0.001)")
    p.add_argument("--output",      default="",                  help="Output ONNX path (default: color_mlp_<transform>.onnx)")
    p.add_argument(
        "--preview",
        metavar="IMAGE",
...
def main() -> None:
    ...
    export_filename = args.output if len(args.output) > 0 else f"color_mlp_{args.transform}.onnx"
