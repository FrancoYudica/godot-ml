...
std::vector<int64_t> ml::TextureInputHandler::upload(
    ...
    ERR_FAIL_COND_V_MSG(_desc.texture.is_null(), {},
                        "Texture2DInputHandler: null texture.");

    Ref<Image> img = _desc.texture->get_image();
    ...
    uint32_t channels = _desc.channels;
    uint32_t pixels = w * h;
    ...
    activations_tm->get_or_create(_desc.tensor_name, shape, floats);
    return shape;
}