...
namespace ml {
    bool OperatorRegistry::init(godot::RenderingDevice* rd) {
        ...
        ERR_FAIL_COND_V_MSG(
            !_register<GemmOperator>(NodeOperator::Gemm, rd), false,
            "OperatorRegistry: failed to register Gemm operator.");
        ERR_FAIL_COND_V_MSG(
            !_register<ReLUOperator>(NodeOperator::ReLU, rd), false,
            "OperatorRegistry: failed to register ReLU operator.");
        ERR_FAIL_COND_V_MSG(
            !_register<SigmoidOperator>(NodeOperator::Sigmoid, rd), false,
